# STATS 220 - Assignment 3 - Sabaoon Raza Khan (skha787)
...
# Q5 - Creating filenames
	filenames <- paste("burglaries", years, months, ".csv", sep="")
	filenames
	
# Q6 - Maximum value across all files
	overallMax <- 0
...
# Q8 - Maximum number of burglaries in Glendowie
	glendowieMax <- 0
	for (item in filenames) {
		content <- read.csv(item)
...
# Q9 - Total number of burglaries in Glendowie
	glendowieTotal <- 0
	for (item in filenames) {
		content <- read.csv(item)
